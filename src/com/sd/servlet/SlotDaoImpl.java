	...
	public boolean isSlotAvailable(String doctorId, Date date, String time) 
	{
		...
		if (session != null) {
			try 
			{
				transaction = session.beginTransaction();
				// get an user object
	            Slot slot = (Slot) session.createQuery("FROM Slot S WHERE S.doctorId = "+doctorId+" AND S.slot_date ="+date+" AND S.slot_time="+time+" AND S.slot_status = \"ACTIVE\"");
	            isSlotAvailable = slot!=null;
	            transaction.commit();
			} catch (Exception exception) {
